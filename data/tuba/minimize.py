...
def handle_line(line, document_state, language, labels, stats):
  ...
  if begin_document_match:
    ...
    document_state.doc_key = get_doc_key(begin_document_match.group(1))
    return None
...
def minimize_language(language, labels, stats):
  extension = "tuba10_gold_conll" if language == "german" else "v4_gold_conll"
  minimize_partition("dev", language, extension, labels, stats)
  minimize_partition("train", language, extension, labels, stats)
  minimize_partition("test", language, extension, labels, stats)

if __name__ == "__main__":
  labels = collections.defaultdict(set)
  stats = collections.defaultdict(int)
  minimize_language("german", labels, stats)
